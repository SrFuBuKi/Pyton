...
def login():
   ...
   return jsonify({'message':"Não autorizado, Credenciais incorretas!"}), 401
...
def get_product_details(product_id):
   ...
   return jsonify({'message':"Não encontrei o Produto"}), 404
...
def remove_from_cart(product_id):
   ...
   return jsonify({'message':"Falha para remover item do carrinho!"}), 400
...
def checkout():
   ...
   for cart_item in cart_items:
      db.session.delete(cart_item)
   db.session.commit()
   return jsonify({'message':"Checkout concluído com sucesso!, Carrinho Limpo"})
